/*
* TelerikReporting v12.2.18.1017 (http://www.telerik.com/products/reporting.aspx)
...
(function(trv, $, window, document, undefined) {
    ...
    function SendEmail(placeholder, options, otherOptions) {
        ...
        var controller = options.controller, initialized = false, viewerOptions = otherOptions, dialogVisible = false, $placeholder, $sendFormPlaceholder, kendoSendEmailDialog, inputFrom, inputTo, inputCC, inputSubject, docFormat, docFormatInput, bodyEditor, bodyText, docFormatList, optionsCommandSet, windowLocation;
        if (!controller) {
        ...
        function ensureInitialized() {
            if (!initialized) {
                ...
                kendoSendEmailDialog = $(".trv-send-email-window").kendoWindow({
                    title: sr.sendEmailDialogTitle,
                    minWidth: 350,
                    minHeight: 350,
                    maxHeight: 900,
                    appendTo: $(".trv-report-viewer").parent() || "body",
                    close: function() {
                        storeDialogPosition();
                        clearValidation();
                    },
                    ...
                    deactivate: function() {
                        toggle(false);
                        clearFields();
                        clearSendEmailToolbarSelection();
                    },
                    activate: function() {
                        setDefaultValues(otherOptions.sendEmail);
                        $(".trv-send-email-fields input[type='email']:visible").first().focus();
                        setTimeout(function() {
                            setValidation();
                        }, 250);
                    }
                }).data("kendoWindow");
                docFormatInput = $("#format");
                docFormat = docFormatInput.kendoComboBox({
                ...
                if ($("#email-body-editor").length) {
                    bodyEditor = $("#email-body-editor").kendoEditor({
        ...
        function adjustDialogPosition() {
            if (!windowLocation) {
                ...
                var padding = 10, kendoWindow = kendoSendEmailDialog.element.parent(".k-window"), left = windowLocation.left, top = windowLocation.top, width = kendoWindow.outerWidth(true), height = kendoWindow.outerHeight(true), right = left + width, bottom = top + height, windowWidth = $(window).innerWidth(), windowHeight = $(window).innerHeight();
                if (otherOptions.selector) {
                    windowWidth = $(otherOptions.selector).innerWidth(), windowHeight = $(otherOptions.selector).innerHeight();
                }
        ...
        function clearSendEmailToolbarSelection() {
            var selectedToolbarSendEmailItem = $(".k-menu").find("a[data-command='telerik_ReportViewer_toggleSendEmailDialog']").closest(".k-item ");
            if (selectedToolbarSendEmailItem.hasClass("k-state-selected")) {
                selectedToolbarSendEmailItem.removeClass("k-state-selected k-state-focused");
            }
        }
        ...
        function validateFields() {
            var fromIsValid = isEmpty(inputFrom) || !isValidEmail(inputFrom), toIsValid = isEmpty(inputTo) || !isValidEmail(inputTo), ccIsValid = inputCC.val().length && !isValidEmail(inputCC), hasFormat = docFormat.value().length;
            if (!hasFormat) {
                showError(docFormatInput, "data-required-msg");
            }
        ...
        function setDefaultValues(sendEmail) {
            ...
            docFormat.trigger("change");
            setBody(sendEmail && sendEmail.body || "");
        }
        function isEmpty($el) {
            if (!$el.val().length) {
...
/* DO NOT MODIFY OR DELETE THIS LINE! UPGRADE WIZARD CHECKSUM C5FE4394AE5562624FED0BF80AD3B059 */